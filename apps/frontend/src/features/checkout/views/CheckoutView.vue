          ...
          <v-card class="checkout-form-card">
            ...
            <v-card-text>
              <v-form ref="checkoutForm" @submit.prevent="handleSubmit">
                <!-- Contact Information -->
                <ContactInformation
                  :is-logged-in="!formData.isGuestOrder"
                  :user-info="null"
                  :errors="validationErrors.guestUserInfo || []"
                  @update="updateGuestUserInfo"
                />
                ...
                <!-- Submit Button -->
                <SubmitButton
                  :is-processing="checkoutStore.loading"
                  :total="cartStore.totalWithShipping"
...
const validateForm = () => {
  ...
  const contactErrors = [];
  if (!formData.value.guestUserInfo?.name)
    contactErrors.push('Name is required');
  if (!formData.value.guestUserInfo?.email)
    contactErrors.push('Email is required');
  if (!formData.value.guestUserInfo?.phone)
    contactErrors.push('Phone is required');
  if (contactErrors.length > 0) errors.guestUserInfo = contactErrors;

  // Check shipping address
  const shippingErrors = [];
  if (!formData.value.shippingAddress?.address)
    shippingErrors.push('Address is required');
  if (!formData.value.shippingAddress?.city)
    shippingErrors.push('City is required');
  if (!formData.value.shippingAddress?.state)
    shippingErrors.push('State is required');
  if (!formData.value.shippingAddress?.country)
    shippingErrors.push('Country is required');
  if (!formData.value.shippingAddress?.postalCode)
    shippingErrors.push('Postal code is required');
...
const handleSubmit = async () => {
  try {
    ...
    const paymentIntentResponse = await checkoutStore.createPaymentIntent(
      formData.value as ICheckout.CreateOrderRequest
    );
    ...
    const confirmResult = await paymentForm.value?.confirmPayment(
      paymentIntentResponse.clientSecret
    );
